...
<?php endif; ?>
    ...
    <div class="flex flex-col gap-1">
        <?php ($sliced = array_slice($queries ?? [], 0, 100)); ?>
        <?php if(!empty($sliced)): ?>
        <?php $__currentLoopData = $sliced; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $query): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php ($connectionName = is_array($query) ? ($query['connectionName'] ?? '') : (is_object($query) ? ($query->connectionName ?? '') : '')); ?>
        <?php ($sql = is_array($query) ? ($query['sql'] ?? '') : (is_object($query) ? ($query->sql ?? '') : '')); ?>
        <?php ($time = is_array($query) ? ($query['time'] ?? '') : (is_object($query) ? ($query->time ?? '') : '')); ?>
        <div
...
<?php /**PATH C:\xampp\htdocs\Attendance Project\Ssendi_Attendance\vendor\laravel\framework\src\Illuminate\Foundation\Providers/../resources/exceptions/renderer/components/query.blade.php ENDPATH**/ ?>