...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('attendance.checkin.store') }}" enctype="multipart/form-data"
                id="checkinForm" class="row g-4">
                @csrf
                    ...
                    <select name="schedule_id" class="form-select" required>
                        ...
                        @forelse($schedules as $schedule)
                            @php
                                ...
                                $isLateByDefault =
                                    $attStatus === 'scheduled' &&
                                    now()->gt($schedule->start_at->copy()->addMinutes(60));
                                $isLate = $attStatus === 'late' || $isLateByThreshold || $isLateByDefault;

                                $suffix = $isClosed ? ' (Disabled)' : ($isLate ? ' (Late Period)' : '');
                            @endphp
                            <option value="{{ $schedule->id }}" {{ $isClosed ? 'disabled' : '' }}>
                                {{ optional($schedule->course)->name }} — {{ $schedule->start_at->format('H:i') }} at
                                {{ $schedule->location }} {{ $suffix }}
                            </option>
                        @empty
                ...
                <div class="col-12">
                    ...
                    <div class="d-flex gap-3">
                        ...
                        <input type="text" name="lng" id="lng" class="form-control" placeholder="Longitude"
                            readonly />
                        <button type="button" id="btnLocate" class="btn btn-outline-secondary">Use current
                            location</button>
                    </div>
    ...
    <script>
        (function() {
            ...
            console.log('[Check-In] Geofence config', {
                campus,
                radiusMeters
            });
            ...
            if (btn) {
                btn.addEventListener('click', function() {
                    ...
                    navigator.geolocation.getCurrentPosition(function(pos) {
                        ...
                        const dist = haversine(campus.lat, campus.lng, pos.coords.latitude, pos.coords
                            .longitude);
                        ...
                        console.log('[Check-In] Current position', {
                            lat: pos.coords.latitude,
                            lng: pos.coords.longitude,
                            dist
                        });
                        if (dist > radiusMeters) {
                            status.textContent = 'Outside campus area (' + Math.round(dist) + 'm).';
                            status.classList.remove('text-success');
                            status.classList.add('text-danger');
                        }
                        // Show quick hint of allowed center and radius
                        const hint =
                            `Allowed center: ${campus.lat.toFixed(6)}, ${campus.lng.toFixed(6)} | Radius: ${radiusMeters}m`;
                        status.title = hint;
                    }, function(err) {
            ...
            if (form) {
                // Compression logic for file input
                const fileInput = form.querySelector('input[name="selfie"]');
                if (fileInput) {
                        ...
                        reader.onload = function(readerEvent) {
                            ...
                            image.onload = function() {
                                ...
                                canvas.toBlob(function(blob) {
                                    ...
                                    console.log('[Check-In] Image compressed', {
                                        original: file.size,
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    distEl.classList.remove('text-success', 'text-danger');
                    distEl.classList.add(dist > radiusMeters ? 'text-danger' : 'text-success');
                    console.log('[Check-In] Submit distance check', {
                        lt,
                        ln,
                        dist,
                        radiusMeters,
                    ...
                    if (dist > radiusMeters) {
                        e.preventDefault();
                        status.textContent =
                            'You are outside the campus area. Attendance can only be recorded within the campus. (' +
                            Math.round(
                                dist) + 'm).';
                        status.classList.remove('text-success');
                        status.classList.add('text-danger');
                    ...
                    fetch(form.action, {
                        ...
                        if (response.ok) {
                            const title = (data && data.message) ? data.message :
                                'Attendance recorded!';
                            const redirectUrl = (data && (data.redirect || data.url)) ? (data
                                .redirect || data.url) : null;
