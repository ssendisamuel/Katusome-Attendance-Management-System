                    ...
                    <form class="row g-4" method="GET" action="{{ route('admin.reports.daily') }}">
                        ...
                        <div class="col-12 col-md-3">
                            ...
                            <select name="course_id" class="form-select">
                                ...
                                @foreach ($courses as $c)
                                    <option value="{{ $c->id }}" @selected(request('course_id') == $c->id)>{{ $c->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12 col-md-3">
                            ...
                            <select name="group_id" class="form-select">
                                ...
                                @foreach ($groups as $g)
                                    <option value="{{ $g->id }}" @selected(request('group_id') == $g->id)>{{ $g->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-12 col-md-6 d-flex align-items-end justify-content-end gap-2 report-actions">
                            <button class="btn btn-outline-primary"><span
                                    class="icon-base ri ri-filter-3-line me-1"></span>Apply Filters</button>
                            ...
                            <button type="button" class="btn btn-outline-danger" data-export="pdf"
                                data-export-target="#dailyTable" data-title="Daily Attendance — {{ $date }}"
                                data-filename="Daily_Attendance_{{ $date }}.pdf" data-header="Katusome Institute"
                                data-footer-left="Katusome • Daily"
                                data-json-url="{{ route('admin.reports.daily.json', ['date' => $date, 'course_id' => request('course_id'), 'group_id' => request('group_id'), 'status' => request('status'), 'search' => request('search')]) }}"><span
                            ...
                            <button type="button" class="btn btn-outline-success" data-export="excel"
                                data-export-target="#dailyTable" data-title="Daily Attendance — {{ $date }}"
                                data-filename="Daily_Attendance_{{ $date }}.xlsx"
                                data-json-url="{{ route('admin.reports.daily.json', ['date' => $date, 'course_id' => request('course_id'), 'group_id' => request('group_id'), 'status' => request('status'), 'search' => request('search')]) }}"><span
                                    class="icon-base ri ri-file-excel-line me-1"></span>Excel</button>
                            <a href="{{ route('admin.reports.daily.csv', ['date' => $date, 'course_id' => request('course_id'), 'group_id' => request('group_id'), 'status' => request('status'), 'search' => request('search')]) }}"
                                class="btn btn-outline-success"><span
                                    class="icon-base ri ri-file-text-line me-1"></span>CSV</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="col-12 col-sm-6 col-lg-12">
                    <div class="card h-100 bg-label-secondary">
                        <div class="card-body d-flex align-items-center gap-3">
                            <div class="avatar avatar-md">
                                <div class="avatar-initial rounded bg-white">
                                    <span class="icon-base ri ri-error-warning-line icon-22px text-secondary"></span>
                                </div>
                            </div>
                            <div>
                                <p class="mb-0">Incomplete (No Clock-Out)</p>
                                <h4 class="mb-0 text-secondary">{{ $incomplete ?? 0 }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                ...
                                                <small
                                                    class="text-muted">{{ optional($row->student)->student_no ?? optional($row->student)->reg_no }}</small>
                                            </div>
                                        </td>
                                            ...
                                            @if ($row->clock_out_time)
                                                ...
                                                @if ($row->is_auto_clocked_out)
                                                    <span
                                                        ...
                                                        title="Auto Clocked Out (Incomplete)">!</span>
                                                @endif
                                            @else
                                                —
                                            @endif
                                        ...
                                        <td>
                                            @php($statusColor = $row->status === 'present' ? 'success' : ($row->status === 'late' ? 'warning' : 'danger'))
                                            <span
                                                class="badge bg-label-{{ $statusColor }}">{{ ucfirst($row->status) }}</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
