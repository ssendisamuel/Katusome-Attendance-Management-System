                    ...
                    <form class="row g-4" method="GET" action="{{ route('admin.reports.individual') }}">
                        <div class="col-12 col-md-4 position-relative">
                            ...
                            <input type="text" id="studentSearch" class="form-control"
                                placeholder="Search student by name or ID" autocomplete="off"
                                value="{{ optional($student)->name }}">
                            <input type="hidden" name="student_id" id="studentId" value="{{ optional($student)->id }}">
                            <div id="studentSuggestions" class="list-group position-absolute w-100"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @switch($row->status)
                                                    ...
                                                    @default
                                                        <span class="badge bg-label-secondary">{{ ucfirst($row->status) }}</span>
                                                @endswitch
                                            </td>
                                        </tr>
                                        @empty
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-2">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button type="button" class="btn btn-outline-danger" data-export="pdf"
                                        data-export-target="#individualTable" data-title="Individual Attendance"
                                        data-filename="Individual_Attendance_{{ optional($student)->name ? \Illuminate\Support\Str::slug($student->name) : 'Unknown' }}.pdf"
                                        data-header="Katusome Institute" data-footer-left="Katusome • Individual"
                                        data-json-url="{{ $student ? route('admin.reports.individual.json', ['student_id' => $student->id]) : '' }}"><span
                                    ...
                                    <button type="button" class="btn btn-outline-success" data-export="excel"
                                        data-export-target="#individualTable" data-title="Individual Attendance"
                                        data-filename="Individual_Attendance_{{ optional($student)->name ? \Illuminate\Support\Str::slug($student->name) : 'Unknown' }}.xlsx"
                                        data-json-url="{{ $student ? route('admin.reports.individual.json', ['student_id' => $student->id]) : '' }}"><span
                                            class="icon-base ri ri-file-excel-line me-1"></span>Excel</button>
                                    @if ($student)
                                        <a class="btn btn-outline-success"
                                            href="{{ route('admin.reports.individual.csv', ['student_id' => $student->id]) }}"><span
