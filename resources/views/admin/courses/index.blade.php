      ...
      <div class="d-flex justify-content-between align-items-center mt-2">
        ...
        <div class="d-flex gap-2">
          <button type="button" class="btn btn-outline-secondary" data-export="print"
            data-export-target="#coursesTableEl"><span class="icon-base ri ri-printer-line me-1"></span>Print</button>
          <button type="button" class="btn btn-outline-danger" data-export="pdf" data-export-target="#coursesTableEl"
            data-title="Courses" data-filename="Courses.pdf" data-header="Katusome Institute"
            data-footer-left="Katusome • Courses"
            data-json-url="{{ route('admin.courses.index', array_merge(request()->query(), ['format' => 'json'])) }}"><span
              class="icon-base ri ri-file-pdf-line me-1"></span>PDF</button>
          <button type="button" class="btn btn-outline-success" data-export="excel" data-export-target="#coursesTableEl"
            data-title="Courses" data-filename="Courses.xlsx"
            data-json-url="{{ route('admin.courses.index', array_merge(request()->query(), ['format' => 'json'])) }}"><span
            ...
            <tr>
              ...
              <td class="text-end">
                ...
                <form action="{{ route('admin.courses.destroy', $course) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-sm btn-outline-danger js-delete-course"
                    data-name="{{ $course->name }}">Delete</button>
                </form>
              </td>
            </tr>
          @endforeach
