    ...
    <ul class="menu-inner py-1">
        ...
        @else
            ...
            @if ($role === 'student')
                ...
                @if (auth()->user()->student && auth()->user()->student->courseLeadership()->exists())
                    <li class="menu-item {{ request()->routeIs('student.course-leader.*') ? 'active' : '' }}">
                        <a href="{{ route('student.course-leader.dashboard') }}" class="menu-link text-warning">
                            <i class="menu-icon icon-base ri ri-star-smile-line"></i>
                            <div>{{ __('Course Leader Panel') }}</div>
                        </a>
                    </li>
                @endif
                <li class="menu-item">
                    ...
                    <form id="logout-form-vertical" action="{{ route('logout') }}" method="POST" class="d-none">@csrf
                    </form>
                </li>
            @elseif($role === 'lecturer')
                <li class="menu-item {{ request()->routeIs('lecturer.dashboard') ? 'active' : '' }}">
                    <a href="{{ route('lecturer.dashboard') }}" class="menu-link">
                        <i class="menu-icon icon-base ri ri-dashboard-line"></i>
                        <div>{{ __('Dashboard') }}</div>
                    </a>
                </li>
                <li class="menu-item {{ request()->routeIs('lecturer.attendance.index') ? 'active' : '' }}">
                    <a href="{{ route('lecturer.attendance.index') }}" class="menu-link">
                        <i class="menu-icon icon-base ri ri-clipboard-line"></i>
                        <div>{{ __('Take Attendance') }}</div>
                    </a>
                </li>
                <li class="menu-item {{ request()->routeIs('lecturer.series.index') ? 'active' : '' }}">
                    <a href="{{ route('lecturer.series.index') }}" class="menu-link">
                        <i class="menu-icon icon-base ri ri-calendar-2-line"></i>
                        <div>{{ __('My Series') }}</div>
                    </a>
                </li>
                <li class="menu-item {{ request()->routeIs('lecturer.schedules.index') ? 'active' : '' }}">
                    <a href="{{ route('lecturer.schedules.index') }}" class="menu-link">
                        <i class="menu-icon icon-base ri ri-calendar-check-line"></i>
                        <div>{{ __('My Schedules') }}</div>
                    </a>
                </li>
